...
interface Message {
    ...
    timestamp: Date
    actions?: Array<{
        type: "create_dataset" | "run_analysis" | "start_synthesis" | "export_report"
        label: string
        data?: any
    }>
...
const mockResponses = {
    创建数据集: {
        ...
        actions: [{ type: "create_dataset", label: "开始创建", data: { step: "config" } }],
    },
    质量分析: {
        content:
            "正在为您分析数据集质量...\n\n📊 **分析结果概览：**\n- 图像分类数据集：质量分 92/100\n- 问答对数据集：质量分 87/100\n- 多模态数据集：质量分 78/100\n\n🔍 **发现的主要问题：**\n- 23个重复图像\n- 156个格式不正确的问答对\n- 78个图文不匹配项\n\n💡 **改进建议：**\n- 建议进行去重处理\n- 优化问答对格式\n- 重新标注图文匹配项",
        actions: [{ type: "run_analysis", label: "查看详细报告", data: { type: "detailed" } }],
    },
    数据合成: {
        content:
            "我可以帮您启动数据合成任务。目前支持以下合成类型：\n\n🖼️ **图像数据合成**\n- 数据增强（旋转、翻转、亮度调整）\n- 风格迁移\n- GAN生成\n\n📝 **文本数据合成**\n- 同义词替换\n- 回译增强\n- GPT生成\n\n❓ **问答对合成**\n- 基于知识库生成\n- 模板变换\n- 多轮对话生成\n\n请告诉我您需要合成什么类型的数据，以及目标数量。",
        actions: [{ type: "start_synthesis", label: "配置合成任务", data: { step: "config" } }],
    },
    导出报告: {
        content:
            "正在为您准备最新的分析报告...\n\n📋 **可用报告：**\n- 数据质量评估报告（PDF）\n- 数据分布统计报告（Excel）\n- 模型性能评估报告（PDF）\n- 偏见检测报告（PDF）\n- 综合分析报告（PDF + Excel）\n\n✅ 报告已生成完成，您可以选择下载格式。",
        actions: [{ type: "export_report", label: "下载报告", data: { format: "pdf" } }],
    },
    查看状态: {
        content:
            "📊 **当前任务状态概览：**\n\n🟢 **运行中的任务：**\n- 问答对生成任务：65% 完成\n- 图像质量分析：运行中\n- 知识库构建：等待中\n\n✅ **已完成的任务：**\n- 图像分类数据集创建：已完成\n- PDF文档提取：已完成\n- 训练集配比任务：已完成\n\n⚠️ **需要关注的任务：**\n- 多模态数据合成：暂停（需要用户确认参数）\n\n所有任务运行正常，预计2小时内全部完成。",
...
interface DataAgentPageProps {
    onBack?: () => void
}

export default function DataAgentPage({ onBack }: DataAgentPageProps) {
    ...
    const generateResponse = (input: string): { content: string; actions?: any[] } => {
        ...
        if (lowerInput.includes("创建") && lowerInput.includes("数据集")) {
            ...
            return {
                ...
                actions: [{ type: "run_analysis", label: "开始分析", data: { query: input } }],
            }
        }
    }
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-purple-100">AI驱动的智能数据助手，通过对话完成复杂数据操作</p>
                                </div>
                            </div>
                            {onBack && (
                                <Button
                                        ...
                                        <div className="space-y-4 pb-4">
                                            {messages.map((message) => (
                                                ...
                                                >
                                                    ...
                                                    <div
                                                        className={`max-w-[80%] rounded-lg px-4 py-3 ${message.type === "user" ? "bg-blue-500 text-white" : "bg-white text-gray-900 shadow-sm"
                                                            }`}
                                                    >
                        ...
                        <div className="space-y-4">
                            <Card className="shadow-lg">
                                <CardHeader className="pb-3">
                                    ...
                                    <CardDescription className="text-sm">点击快速开始常用操作</CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-2">
                                    {quickActions.map((action) => (
                                        <Button
                                            key={action.id}
                                            variant="outline"
                                            className="w-full justify-start h-auto p-3 bg-transparent hover:bg-purple-50"
