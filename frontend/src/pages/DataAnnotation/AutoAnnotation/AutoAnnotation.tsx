      ...
      Modal.info({
        ...
        content: (
          <div>
            ...
            <p>处理图片数：{task.processedImages} / {task.totalImages}</p>
          </div>
        ),
      });
    }
  ...
  const columns: ColumnType<AutoAnnotationTask>[] = [
    ...
    },
    {
      ...
      render: (classes: number[]) => (
        <Tooltip title={classes.length > 0 ? classes.join(", ") : "全部类别"}>
          <span>{classes.length > 0 ? `${classes.length} 个类别` : "全部类别"}</span>
        </Tooltip>
      ),
    },
    {
