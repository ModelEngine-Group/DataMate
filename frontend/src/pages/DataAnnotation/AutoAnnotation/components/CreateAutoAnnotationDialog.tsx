...
// COCO 80 类，需与后端 COCO_CLASS_MAP 保持一致
const COCO_CLASSES = [
    ...
    try {
      ...
      const payload = {
        ...
        config: {
          ...
          // 可选的新数据集名称，后端通过 config.outputDatasetName 接收
    ...
    >
      ...
      >
        ...
        <Form.Item
          ...
          label="新数据集名称（可选）"
          tooltip="如果填写，后续可将自动标注结果注册为该名称的新数据集"
          rules={[{ max: 255, message: "数据集名称不能超过255个字符" }]}
        >
          <Input placeholder="例如：Flicker_auto_labeled" />
        </Form.Item>

        <Form.Item
          name="modelSize"
          label="模型规模"
          tooltip="模型越大精度越高，但速度越慢"
          rules={[{ required: true, message: "请选择模型规模" }]}
        >
          <Select placeholder="请选择模型规模">
            <Option value="n">YOLOv8n - 最快（精度较低）</Option>
            <Option value="s">YOLOv8s - 快速</Option>
            <Option value="m">YOLOv8m - 平衡</Option>
            <Option value="l">YOLOv8l - 推荐（精度高）</Option>
            <Option value="x">YOLOv8x - 最精确（速度慢）</Option>
          </Select>
        </Form.Item>
          ...
          {!selectAllClasses && (
            <Form.Item
              ...
              rules={[
                {
                  validator: (_, value) => {
                    if (!selectAllClasses && (!value || value.length === 0)) {
                      return Promise.reject(new Error("请至少选择一个目标类别"));
                    }
