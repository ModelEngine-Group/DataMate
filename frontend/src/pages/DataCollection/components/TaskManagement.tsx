...
import { Card, Button, Input, Badge, Table, Select, Dropdown, Menu } from "antd"
import { SearchOutlined, MoreOutlined, PlayCircleOutlined, StopOutlined, EditOutlined, DeleteOutlined } from "@ant-design/icons"
...
export default function TaskManagement() {
    ...
    const handleStartTask = (taskId: string) => {
        setTasks(tasks.map((task) => (task.id === taskId ? { ...task, status: "running" as const } : task)))
    }

    const handleStopTask = (taskId: string) => {
        setTasks(tasks.map((task) => (task.id === taskId ? { ...task, status: "stopped" as const } : task)))
    }
                ...
                <Dropdown
                    overlay={
                        <Menu>
                            ...
                            <Menu.Item key="delete" icon={<DeleteOutlined />} danger onClick={() => handleDeleteTask(record.id)}>
                                删除
                            </Menu.Item>
                        </Menu>
                    }
