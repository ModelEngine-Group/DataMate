...
export default function DataEvaluationPage() {
    ...
    const filteredTasks = tasks.filter((task) => {
        ...
        // 评估方式过滤
        if (selectedFilters.evaluationType?.length && !selectedFilters.evaluationType.includes(task.evaluationType)) {
            return false
        }

        // 状态过滤
        if (selectedFilters.status?.length && !selectedFilters.status.includes(task.status)) {
            return false
        }

        // 数据集过滤
        if (selectedFilters.dataset?.length && !selectedFilters.dataset.includes(task.datasetId)) {
            return false
        }
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
                    <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>数据评估</h1>
                    <div style={{ color: "#888", marginTop: 4 }}>管理和监控数据集的质量评估任务</div>
                </div>
                <Button
                    type="primary"
                    icon={<PlusOutlined />}
            ...
            {viewMode === "card" ? (
                <CardView
                    data={sortedTasks.map((task) => ({
                        ...
                        icon: task.evaluationType === "model"
                        ...
                        statistics: [
                            { label: "进度", value: task.progress !== undefined ? `${task.progress}%` : "-" },
                        ...
                        columns={[
                            {
                                ...
                                render: (text, record) => (
                                    <div>
                                        ...
                                        <div style={{ fontSize: 13, color: "#888" }}>{record.description}</div>
                                    </div>
                                ),
                            },
                            {
                            ...
                            },
                            {
                                ...
                                render: (type) => (
                                    <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                                        ...
                                        <span style={{ fontSize: 13 }}>{type === "model" ? "模型评估" : "人工评估"}</span>
                                    </div>
                                ),
                            },
                            {
                            ...
                            },
                            {
                                ...
                                render: (score) =>
                                    score ? (
                                        <span style={{ fontWeight: 500, color: "#389e0d" }}>{score}分</span>
                                    ) : (
                                        <span style={{ fontSize: 13, color: "#bbb" }}>-</span>
                                    ),
                            },
                            ...
                            },
                            {
                                ...
                                render: (_, task) => (
                                    <div style={{ display: "flex", gap: 8 }}>
                                        ...
                                        {task.evaluationType === "manual" && task.status === "pending" && (
                                            <Button
                                                type="default"
                                                size="small"
                                                icon={<EditOutlined />}
                        ...
                        locale={{
                            emptyText: (
                                <div style={{ textAlign: "center", padding: 48, color: "#bbb" }}>
                                    ...
                                    <div style={{ fontSize: 13, color: "#ccc" }}>点击"创建评估任务"开始评估数据集质量</div>
                                </div>
                            ),
                        }}
                    />
            ...
            {sortedTasks.length === 0 && (
                <div style={{ textAlign: "center", padding: "48px 0" }}>
                    <DatabaseOutlined style={{ fontSize: 64, color: "#bbb", marginBottom: 16 }} />
                    <div style={{ fontSize: 18, fontWeight: 500, marginBottom: 8 }}>暂无评估任务</div>
                    <div style={{ color: "#888", marginBottom: 24 }}>创建您的第一个数据评估任务</div>
                    <Button
                        type="primary"
                        icon={<PlusOutlined />}
                        onClick={() => navigate("/data/evaluation/create-task")}
