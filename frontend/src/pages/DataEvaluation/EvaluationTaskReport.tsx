...
import { mockQAPairs, mockTasks, presetEvaluationDimensions } from "@/mock/evaluation"
...
const EvaluationTaskReport = () => {
    ...
    const getTaskAllDimensions = (task: EvaluationTask) => {
        const presetDimensions = presetEvaluationDimensions.filter((d) => task.dimensions.includes(d.id))
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            <div className="m-4 mx-auto space-y-6">
                {/* 头部 */}
                <Card className="!bg-white !rounded-lg !shadow-sm !p-6">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            <Button className="flex items-center gap-2" icon={<Download className="w-4 h-4" />}>
                                导出报告
                            </Button>
                        </div>
                    </div>
                    ...
                    <Card>
                        <div className="p-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="text-2xl font-bold text-gray-900">{selectedTask.score || 0}</div>
                    ...
                    <Card>
                        <div className="p-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="text-2xl font-bold text-gray-900">{selectedTask.sliceConfig?.sampleCount}</div>
                    ...
                    <Card>
                        <div className="p-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="text-2xl font-bold text-gray-900">{selectedTask.progress}%</div>
                                        ...
                                        <div key={dimension.id} className="flex items-center justify-between">
                                            <div className="flex-1">
                                                <div className="flex items-center justify-between mb-1">
                                                    ...
                                                    <span className="text-sm font-bold text-blue-600">{score}分</span>
                                                </div>
                        ...
                        <div className="space-y-4 mt-4">
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    ...
                                    <span className="ml-2 font-medium">{selectedTask.sliceConfig?.threshold}</span>
                                </div>
                                <div>
                                    <span className="text-gray-500">抽样数量:</span>
                                    <span className="ml-2 font-medium">{selectedTask.sliceConfig?.sampleCount}</span>
                                </div>
                                <div>
                                    <span className="text-gray-500">切片方法:</span>
                                    <span className="ml-2 font-medium">{selectedTask.sliceConfig?.method}</span>
                                </div>
                                <div>
                                    <span className="text-gray-500">评估时间:</span>
                                    <span className="ml-2 font-medium">{selectedTask.completedAt || selectedTask.createdAt}</span>
                                </div>
                            </div>
                            ...
                            <div className="border-t pt-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {allDimensions.map((dimension) => (
                                        <Badge key={dimension.id} style={{ border: "1px solid #d9d9d9", background: "#fafafa", padding: "0 8px" }}>
                                            {dimension.name}
                                        </Badge>
                                    ))}
                                </div>
                    ...
                    <div className="space-y-4 mt-4">
                        {mockQAPairs.map((qa) => (
                            <div key={qa.id} className="border rounded-lg p-4">
                                <div className="space-y-3">
                                    <div>
                                        <span className="text-sm font-medium text-gray-700 mb-1">问题:</span>
                                        <span className="text-gray-900">{qa.question}</span>
                                    </div>
                                    <div>
                                        <span className="text-sm font-medium text-gray-700 mb-1">回答:</span>
                                    ...
                                    <div className="flex items-center justify-between pt-2 border-t">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="flex items-center gap-1">
                                                {[1, 2, 3, 4, 5].map((star) => (
                                                    <Star
                                                        key={star}
                                                        className={`w-4 h-4 ${star <= qa.score ? "text-yellow-400" : "text-gray-300"}`}
                                        ...
                                        {qa.feedback && <div className="text-sm text-gray-600">{qa.feedback}</div>}
                                    </div>
                                </div>
                            </div>
                        ))}
