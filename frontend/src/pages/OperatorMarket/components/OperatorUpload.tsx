...
import { Upload, FileText, CheckCircle, Settings, TagIcon, Plus, X, Eye } from "lucide-react"
...
export default function OperatorUploadPage() {
    const [uploadStep, setUploadStep] = useState<"upload" | "parsing" | "configure" | "preview">("upload")
                ...
                if (prev >= 100) {
                    ...
                    setTimeout(() => {
                        setParsedInfo({
                            name: "图像预处理算子",
                            version: "1.2.0",
                            description: "支持图像缩放、裁剪、旋转、颜色空间转换等常用预处理操作，优化了内存使用和处理速度",
                            author: "当前用户",
                            category: "图像处理",
                            modality: ["image"],
                            type: "preprocessing",
                            ...
                            language: "Python",
                            size: "2.3MB",
                            dependencies: ["opencv-python>=4.5.0", "pillow>=8.0.0", "numpy>=1.20.0"],
        ...
        <div className="w-full mx-auto">
            <Card className="text-center">
                <div className="py-12">
                    ...
                    <p className="text-gray-600 mb-8">支持多种格式的算子文件，系统将自动解析配置信息</p>
                    ...
                    >
                        {isUploading ? (
                            ...
                            <div>
                                ...
                                <p className="text-lg text-gray-600 mb-2">拖拽文件到此处或点击选择文件</p>
                                <p className="text-sm text-gray-500">支持单个文件或多个文件同时上传</p>
                            </div>
                        )}
                    </div>
                </div>
        ...
        <div className="w-full mx-auto">
            <Card>
                <div className="text-center py-12">
                    ...
                    <h2 className="text-2xl font-bold text-gray-900 mb-4">正在解析算子文件</h2>
                    <p className="text-gray-600 mb-8">系统正在自动分析您的算子文件，提取配置信息...</p>
                    ...
                    <div className="max-w-md mx-auto">
                        <Progress percent={parseProgress} status="active" strokeColor="#3B82F6" />
    ...
    const renderConfigureStep = () => (
        <div className="w-full mx-auto space-y-6">
            ...
            <Card>
                ...
                {parseError && (
                    <Alert message="解析过程中发现问题" description={parseError} type="warning" showIcon className="mb-6" />
                )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">算子名称</label>
                                    <div className="p-2 bg-gray-50 rounded border text-gray-900">{parsedInfo.name}</div>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">版本</label>
                                    <div className="p-2 bg-gray-50 rounded border text-gray-900">{parsedInfo.version}</div>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">作者</label>
                                    <div className="p-2 bg-gray-50 rounded border text-gray-900">{parsedInfo.author}</div>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">分类</label>
                                    <div className="p-2 bg-gray-50 rounded border text-gray-900">{parsedInfo.category}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <div className="p-2 bg-gray-50 rounded border text-gray-900">{parsedInfo.framework}</div>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">语言</label>
                                    <div className="p-2 bg-gray-50 rounded border text-gray-900">{parsedInfo.language}</div>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">类型</label>
                                    <div className="p-2 bg-gray-50 rounded border text-gray-900">{parsedInfo.type}</div>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">模态</label>
                                    <div className="p-2 bg-gray-50 rounded border text-gray-900">{parsedInfo.modality.join(", ")}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="md:col-span-2">
                            ...
                            <div className="p-3 bg-gray-50 rounded border text-gray-900">{parsedInfo.description}</div>
                        </div>

                        {/* 依赖项 */}
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">依赖项</label>
                        ...
                        {/* 性能指标 */}
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">性能指标</label>
                            <div className="p-3 bg-gray-50 rounded border space-y-2">
                                {parsedInfo.performance.accuracy && (
                                    <div className="text-sm">
                                        <span className="font-medium">准确率:</span> {parsedInfo.performance.accuracy}%
                                    </div>
                                )}
                                <div className="text-sm">
                                    <span className="font-medium">速度:</span> {parsedInfo.performance.speed}
                                </div>
                                <div className="text-sm">
                                    <span className="font-medium">内存:</span> {parsedInfo.performance.memory}
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <Card>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {availableTags.map((tag) => (
                            <button
                                ...
                                className={`px-3 py-1 rounded-full text-sm font-medium border transition-colors ${selectedTags.includes(tag)
                ...
                {/* 自定义标签 */}
                <div className="mb-6">
                    <h3 className="text-lg font-semibold text-gray-900 mb-3">添加自定义标签</h3>
                ...
                {selectedTags.length > 0 && (
                    <div>
                        <h3 className="text-lg font-semibold text-gray-900 mb-3">已选标签 ({selectedTags.length})</h3>
                        <div className="flex flex-wrap gap-2">
                ...
                <div className="flex justify-end gap-3 mt-8">
                    ...
                    <Button type="primary" onClick={handlePublish} disabled={selectedTags.length === 0}>
                        发布算子
                    </Button>
                </div>
            </Card>
    ...
    const renderStepIndicator = () => (
        <div className="w-full p-4">
            <Steps
                ...
                current={uploadStep === "upload" ? 0 : uploadStep === "parsing" ? 1 : uploadStep === "configure" ? 2 : 3}
            />
        </div>
    )
