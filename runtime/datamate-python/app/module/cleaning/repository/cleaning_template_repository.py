...
class CleaningTemplateRepository:
    ...
    async def find_all_templates(
        ...
        """Query all templates with optional pagination"""
        query = select(self.model)

        if keyword:
            keyword_pattern = f"%{keyword}%"
            query = query.where(
                self.model.name.ilike(keyword_pattern)
    ...
    async def count_templates(
        self, db: AsyncSession, keyword: Optional[str] = None
    ) -> int:
        """Count templates"""
    ...
    async def insert_template(
        self, db: AsyncSession, template: CleaningTemplate
    ) -> None:
        """Insert new template"""
        db.add(template)
        await db.flush()
