...
class KnowledgeBaseCreateReq(BaseModel):
    ...
    type: RagType = Field(default=RagType.DOCUMENT, description="RAG 类型")
    embedding_model: str = Field(
        ..., min_length=1, alias="embeddingModel", description="嵌入模型ID"
    )
...
class KnowledgeBaseUpdateReq(BaseModel):
    ...
    class Config:
        populate_by_name = True
        json_schema_extra = {
            "example": {"name": "updated_knowledge_base", "description": "更新后的描述"}
        }
...
class KnowledgeBaseQueryReq(BaseModel):
    ...
    page_size: int = Field(
        default=10, ge=1, le=100, alias="size", description="每页数量"
    )
    ...
    type: Optional[RagType] = Field(None, description="按 RAG 类型筛选")

    class Config:
        populate_by_name = True
        json_schema_extra = {
            "example": {"page": 1, "size": 10, "name": "测试", "type": "DOCUMENT"}
        }
...
class FileInfo(BaseModel):
    ...
    class Config:
        populate_by_name = True
        json_schema_extra = {
            "example": {"id": "file-uuid-123", "fileName": "document.pdf"}
        }
...
class AddFilesReq(BaseModel):
    ...
    knowledge_base_id: Optional[str] = Field(
        ...
        description="知识库ID（从路径参数获取，这里保留用于兼容）",
    )
    ...
    chunk_size: int = Field(
        default=500, ge=50, le=2000, alias="chunkSize", description="分块大小"
    )
    overlap_size: int = Field(
        default=50, ge=0, le=500, alias="overlapSize", description="重叠大小"
    )
    delimiter: Optional[str] = Field(
        None, description="自定义分隔符（仅用于 CUSTOM_SEPARATOR_CHUNK）"
    )
    files: List[FileInfo] = Field(..., min_length=1, description="文件列表")
    ...
    def validate_delimiter(cls, v, info):
        """验证自定义分隔符"""
        if (
            info.data.get("process_type") == ProcessType.CUSTOM_SEPARATOR_CHUNK
            and not v
        ):
...
class DeleteFilesReq(BaseModel):
    ...
    file_ids: List[str] = Field(
        ..., min_length=1, alias="ids", description="要删除的文件ID列表"
    )

    class Config:
        populate_by_name = True
        json_schema_extra = {"example": {"ids": ["file-1", "file-2", "file-3"]}}
...
class RagFileReq(BaseModel):
    ...
    status: Optional[FileStatus] = Field(None, description="按状态筛选")
...
class RetrieveReq(BaseModel):
    ...
    threshold: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="相似度阈值（仅返回分数大于等于该值的结果）"
    )
    knowledge_base_ids: List[str] = Field(
        ..., min_length=1, alias="knowledgeBaseIds", description="要检索的知识库ID列表"
    )
...
class QueryRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"knowledge_base_id": "kb-uuid-123", "query": "什么是机器学习？"}
        }


class ImageRetrieveReq(BaseModel):
    """图片检索请求

    对应 Java: com.datamate.rag.indexer.interfaces.dto.ImageRetrieveReq
    """
    ...
    top_k: int = Field(
        default=10, ge=1, le=100, alias="topK", description="返回结果数量"
    )
    knowledge_base_ids: List[str] = Field(
        ...,
        min_length=1,
        alias="knowledgeBaseIds",
        description="知识库 ID 列表（目前只支持单个知识库）",
    )
