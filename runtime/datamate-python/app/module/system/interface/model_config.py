...
from app.module.system.service import model_config_service
...
async def get_providers():
    ...
    data = await model_config_service.get_providers()
...
async def get_models(
    ...
    data = await model_config_service.get_models(db, q)
    return StandardResponse(code=200, message="success", data=data)


@router.post("/create", response_model=StandardResponse[ModelConfigResponse])
async def create_model(req: CreateModelRequest, db: AsyncSession = Depends(get_db)):
    """创建模型配置，与 Java POST /models/create 一致。"""
    data = await model_config_service.create_model(db, req)
    return StandardResponse(code=200, message="success", data=data)
...
async def get_model_detail(model_id: str, db: AsyncSession = Depends(get_db)):
    ...
    data = await model_config_service.get_model_detail(db, model_id)
    return StandardResponse(code=200, message="success", data=data)
...
async def update_model(
    ...
    data = await model_config_service.update_model(db, model_id, req)
    return StandardResponse(code=200, message="success", data=data)
...
async def delete_model(model_id: str, db: AsyncSession = Depends(get_db)):
    ...
    await model_config_service.delete_model(db, model_id)
