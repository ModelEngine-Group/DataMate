...
| maxConcurrentFiles | inputNumber | 1 | 同一容器内同时运行 ASR 推理的文件数，默认 1 避免多 worker 同时加载模型导致 OOM |
...
ASR 会加载较大的 WeNet 模型。DataMate/Ray 可能为同一算子启动多个 worker；如果多个 worker 同时识别音频，容易触发容器 OOM。`maxConcurrentFiles` 默认设为 1，会在算子内部把真正的 ASR 推理串行化；如果部署环境内存充足，可以调大到 2-4。
