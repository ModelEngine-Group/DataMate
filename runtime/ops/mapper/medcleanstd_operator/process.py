...
class MedCleanStdMapper(Mapper):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # NER schema 从 checkbox 获取，转换为列表
        ...
        self.max_sentences = int(kwargs.get('max_sentences', 10))
        self.normalizer_batch_size = int(kwargs.get('normalizer_batch_size', 24))
    ...
    def _init_components(self):
        ...
        try:
            ...
            self._ner = SiameseNER(model_dir=model_dir)
    ...
    def _split_by_sentences(self, text: str, max_sentences: int = 10) -> List[str]:
        """
    ...
    def execute(self, sample: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 添加 sourceFileSize 字段（平台框架必需）
            # 如果输入是文件路径，获取文件大小；否则使用文本长度
            if os.path.exists(input_text):
            ...
            sample['entities'] = []
            # 确保 sourceFileSize 不为 None（使用文本长度或 0）
            try:
                input_text = sample.get('text', '')
                if os.path.exists(input_text):
                    sample['sourceFileSize'] = os.path.getsize(input_text)
